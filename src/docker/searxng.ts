// src/docker/searxng.ts - SearXNG Docker container management with auto-discovery
 ...
 */
export async function ensureSearxngRunning(): Promise<{
  url: string;
  healthy: boolean;
  autoManaged: boolean;
}> {
  ...
  if (!dockerAvailable) {
    console.error('Docker not available. SearXNG auto-start disabled.');
    return {
      url: config.searxngUrl || `http://localhost:${DEFAULT_PORT}`,
      healthy: false,
      autoManaged: false,
    };
  }
  ...
  if (existing) {
    console.error(`Found existing SearXNG container: ${existing.containerId.slice(0, 12)}`);
    
    if (existing.status === 'running') {
      console.error(`Container is already running at ${existing.url}`);
    ...
    console.error('Starting existing container...');
    const started = await startExistingContainer(existing.containerId);
    
    if (started) {
      console.error(`Waiting for SearXNG to be ready at ${started.url}...`);
  ...
  console.error('Creating new SearXNG container...');
  const created = await createSearxngContainer();
  
  if (created) {
    console.error(`Waiting for SearXNG to be ready at ${created.url}...`);
